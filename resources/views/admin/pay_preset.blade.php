            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <hr>
                        <div>
                            <select id="course" class="selectd" onchange="filter_preset(this.value, 'pay')">
                        ...
                        <form class="forms-sample" method="post" action="{{ url('admin/save_pay/' . $preset->presetid) }}">
                            @csrf
                                ...
                                <table class="table table-bordered" id="dynamic_field">
                                    @if (!empty($page_data))
                                        @foreach ($page_data as $key => $data)
                                            <tr id="row{{ $key + 1 }}">
                                                <td><span>Between $</span><input type="number" name="from[]"
                                                        placeholder="00" class="selectd name_list" required
                                                        value="{{ $data->from }}" /></td>
                                                <td><span>To $</span><input type="number" name="to[]" placeholder="00"
                                                        class="selectd name_list to-input" required
                                                        value="{{ $data->to }}" /></td>
                                                <td>
                                                    <select name="addless[]" id="" class="selectd" required>
        ...
        $(document).ready(function() {
            ...
            $('#add').click(function() {
                ...
                $('#dynamic_field').append('<tr id="row' + i +
                    '" class="dynamic-added"><td><span>Between $</span><input ' + readonly +
                    ' value="' +
                    newFromValue +
                    '" type="number" name="from[]" placeholder="00" class="selectd name_list" required /></td><td><span>To $</span><input type="number" name="to[]" placeholder="00" class="selectd name_list to-input" required /></td> <td><select class="selectd" required name="addless[]" id=""><option value="-">Deduct</option><option value="+">Add</option></select></td><td><input type="number" name="point[]" placeholder="00" class="selectd name_list" required /><span>Points</span></td><td><button type="button" name="remove" id="' +
