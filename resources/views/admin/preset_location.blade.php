            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <hr>
                        <div>
                            <select id="course" class="selectd" onchange="filter_preset(this.value, 'location')">
                        ...
                        <form class="forms-sample" method="post"
                            action="{{ url('admin/save_location_preset/' . $preset->presetid) }}">
                            @csrf
                                ...
                                <table class="table table-bordered" id="dynamic_field">
                                    @if (!empty($page_data))
                                        @foreach ($page_data as $key => $data)
                                            <tr>
                                                <td class="ascca"><span>Location</span>{!! location_select_box('admin', $data->locid) !!}</td>
                                                <td>
                                                <select name="addless[]" id="" required class="selectd">
                                                    <option {{ $data->addless == '-' ? 'selected' : '' }} value="-">
                                                        Deduct</option>
                                    ...
                                    @else
                                        <tr>
                                            <td><span>Location</span>{!! location_select_box('admin') !!}</td>
                                            <td>
        ...
        $(document).ready(function() {
            ...
            $('#add').click(function() {
                ...
                $('#dynamic_field').append('<tr id="row' + i +
                    '" class="dynamic-added"><td  class="ascca"><span>Location</span>{!! location_select_box("admin") !!}</td><td><select class="selectd" required name="addless[]" id=""><option value="-">Deduct</option><option value="+">Add</option></select></td><td><input type="number" name="point[]" placeholder="00" class="selectd name_list" required /><span>Points</span></td><td><button type="button" name="remove" id="' +
