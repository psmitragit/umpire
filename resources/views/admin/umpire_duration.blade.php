            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <hr>
                        <div>
                            <select  class="selectd" id="course" class="selectd"
                                onchange="filter_preset(this.value, 'umpire-duration')">
                        ...
                        <form class="forms-sample" method="post"
                            action="{{ url('admin/save_umpire_duration/' . $preset->presetid) }}">
                            @csrf
                                    ...
                                    <table class="table table-bordered" id="dynamic_field">
                                        @if (!empty($page_data))
                                            @foreach ($page_data as $key => $data)
                                                <tr id="row{{ $key + 1 }}">
                                                    <td><input type="number" name="from[]" placeholder="00"
                                                            class="selectd name_list" required
                                                            value="{{ $data->from }}" /><span>Days To</span></td>
                                                    <td><input type="number" name="to[]" placeholder="00"
                                                            class="selectd name_list to-input" required
                                                            value="{{ $data->to }}" /><span>Days</span></td>
                                                    <td>
                                                    ...
                                                    <td><input type="number" name="point[]" placeholder="00"
                                                            class="selectd name_list" required
                                                            value="{{ $data->point }}" /><span class="ms-2">Points</span></td>
        ...
        $(document).ready(function() {
            ...
            $('#add').click(function() {
                ...
                $('#dynamic_field').append('<tr id="row' + i +
                    '" class="dynamic-added"><td><input ' + readonly + ' value="' +
                    newFromValue +
                    '" type="number" name="from[]" placeholder="00" class="selectd name_list" required /><span>Days To</span></td><td><input type="number" name="to[]" placeholder="00" class="selectd name_list to-input" required /><span>Days</span></td> <td><select  class="selectd" required name="addless[]" id=""><option value="-">Deduct</option><option value="+">Add</option></select></td><td><input type="number" name="point[]" placeholder="00" class="selectd name_list" required /><span class="ms-2">Points</span></td><td><button type="button" name="remove" id="' +
