    ...
    <div class="body-content">
        ...
        <div class="namevs-anes">
            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    <form class="forms-sample form" method="post" action="{{ url('league/save_time') }}">
                        @csrf
                            ...
                            @if (!empty($page_data))
                                @foreach ($page_data as $key => $data)
                                    <section class="displayflesxsa tegss age" id="row{{ $key + 1 }}">
                                        <div class="d-flex vsa">
                                            <span class="text1s ">If game starts between</span>
                                            <span class="pists">{!! generateHourSelectBox('from[]', $data->from) !!}</span>
                                            <span class="tos">&</span>
                                            <span class="pists">{!! generateHourSelectBox('to[]', $data->to) !!}</span>
                                        </div>
                                        <div class="d-flex vsa">
                                            ...
                                            <span class="pists"><input name="point[]" type="number" class="name-inpostya"
                                                    value="{{ $data->point }}" required></span>
        ...
        $(document).ready(function() {
            ...
            $('#add').click(function() {
                ...
                var html = '<section class="displayflesxsa tegss age" id="row' + i +
                    '">';
                html += '<span class="text1s ">If game starts between</span>';
                html += '<span class="pists">';
                html += '{!! generateHourSelectBox('from[]') !!}';
                html += '</span><span class="tos">&</span><span class="pists">';
                html += '{!! generateHourSelectBox('to[]') !!}';
